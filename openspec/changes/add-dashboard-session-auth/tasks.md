...
- [ ] 1.5 Update `lib/family-auth.ts` (and any shared helpers) so API routes call `getServerSession` instead of the shared header; ensure every `app/api/*` route rejects requests without a valid session and conveys clear 401/403 responses, while still honoring demo user fallbacks if appropriate.
