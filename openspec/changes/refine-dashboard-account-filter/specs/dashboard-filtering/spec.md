...
### Requirement: Selected accounts drive the displayed transactions
The system SHALL include every selected account ID as an `accountId` query parameter when fetching `/api/transactions` so that the table only renders rows that belong to the chosen accounts.

#### Scenario: Filtering by accounts refreshes the table
- **WHEN** the user selects a subset of linked accounts via the multi-select or clears the selection back to “all,” the dashboard MUST reset pagination/select state, rerun the `TransactionsTable` query with the matching account IDs, and leave the summary data in sync with those rows
- **THEN** the number of displayed transactions, selection count, and summary totals SHALL only reflect the accounts that remain selected, and clearing all selections SHOULD send the `"all"` sentinel so every account appears again
